...
def load_data(validation_split=0.8):
    train_df = pd.read_pickle(DATA_ROOT + 'train-all-cc.pkl')
    test_df = pd.read_pickle(DATA_ROOT + 'test-all-cc.pkl')
...
def model():
    ...
    with open(DATA_ROOT + 'model_cc_all.json', 'w') as f:
        f.write(model_json)
    ...
    model_path = DATA_ROOT + 'hierarchical_cc_all.hdf5'
    ...
    fs = 0.0
    n = 0
    with open(DATA_ROOT + 'predictions-cc-all.txt', 'w') as f:
