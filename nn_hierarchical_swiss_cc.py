...
def load_data(validation_split=0.8):
    train_df = pd.read_pickle(DATA_ROOT + 'train-mouse-cc.pkl')
    test_df = pd.read_pickle(DATA_ROOT + 'test-mouse-cc.pkl')
...
def model():
    ...
    with open(DATA_ROOT + 'model_cc_mouse.json', 'w') as f:
        f.write(model_json)
    ...
    model_path = DATA_ROOT + 'hierarchical_cc_mouse.hdf5'
    ...
    fs = 0.0
    n = 0
    with open(DATA_ROOT + 'predictions-cc-mouse.txt', 'w') as f:
