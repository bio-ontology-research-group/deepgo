...
func_df = pd.read_pickle(DATA_ROOT + 'mf-yeast.pkl')
...
def load_data(validation_split=0.8):
    train_df = pd.read_pickle(DATA_ROOT + 'train-yeast-mf.pkl')
    test_df = pd.read_pickle(DATA_ROOT + 'test-yeast-mf.pkl')
...
def model():
    ...
    with open(DATA_ROOT + 'model_mf_yeast.json', 'w') as f:
        f.write(model_json)
    ...
    model_path = DATA_ROOT + 'hierarchical_mf_yeast.hdf5'
    ...
    fs = 0.0
    n = 0
    with open(DATA_ROOT + 'predictions-mf-yeast.txt', 'w') as f:
