...
func_df = pd.read_pickle(DATA_ROOT + 'bp-mouse.pkl')
...
def load_data(validation_split=0.8):
    train_df = pd.read_pickle(DATA_ROOT + 'train-mouse-bp.pkl')
    test_df = pd.read_pickle(DATA_ROOT + 'test-mouse-bp.pkl')
...
def model():
    ...
    with open(DATA_ROOT + 'model_bp_mouse.json', 'w') as f:
        f.write(model_json)
    ...
    model_path = DATA_ROOT + 'hierarchical_bp_mouse.hdf5'
    ...
    fs = 0.0
    n = 0
    with open(DATA_ROOT + 'predictions-bp-mouse.txt', 'w') as f:
